...
Un seul fichier à modifier pour rendre le site opérationnel : **`js/app.js`**, tout en haut.
...
```
index.html          → la page (structure des sections)
css/style.css        → tous les styles (couleurs, typographie, mise en page)
js/app.js             → configuration produits + logique (numéro WhatsApp, prix)
manifest.json         → identité de l'app installable (PWA)
sw.js                  → mise en cache hors-ligne
vercel.json            → règles de cache pour Vercel (évite que sw.js reste bloqué en cache)
...
Dans `js/app.js`, duplique un bloc du tableau `PRODUCTS` et ajuste
`id`, `name`, `quote`, `price` et `variants` (une entrée par coloris,
avec le chemin de l'image dans `assets/products/`).
